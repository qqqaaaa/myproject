...
public class InfluxDBClientTest {
    ...
    public void batchInsert1(List<Map> data) throws Exception {
        BatchPoints batchPoints = BatchPoints.database(database).build();
        for(int i = 0;i<data.size();i++){
            Map m = data.get(i);
            Point point = null;
            ...
            String type = m.get("type").toString();
            String measurement = m.get("deviceId").toString();
            int value = Integer.parseInt(m.get("value").toString());
            point = Point.measurement(measurement)
                    .tag("ID", id)
                    .tag("name",name)
                    .tag("type",type)
                    .time(System.currentTimeMillis(), TimeUnit.MILLISECONDS)
                    .addField("int_value",value).addField("int_value1",value).addField("int_value2",value).addField("int_value3",value).addField("int_value4",value).addField("int_value5",value).addField("int_value6",value).addField("int_value7",value).addField("int_valu8",value).addField("int_value9",value).build();
            batchPoints.point(point);
        }
        logger.error("写入数量：" +batchPoints.getPoints().size());
        influxDB.write(batchPoints);
